...
export default function LimitSell({ userData, setError, otherInfo, orderLeverage }) {
    ...
    return (
        <>
            <div className="w-full h-full SkuuBit-DefaultContainer flex flex-col gap-6 bg-red-500">
                ...
                <button className="SkuuBit-SellButton" onClick={
                    async () => {
                        setOpenOrderLoading(true);
                        const newOrder = await OpenTrade(userData.uid, otherInfo + "USDT", quantityQuery, priceQuery, 'sell', 'limit', 'open', orderLeverage, takeProfitPrice, stopLossPrice);
                        if (newOrder.result) {
                    ...
                    <h1 className="text-white">{(openOrderLoading ? "Caricamento..." : "Short")}</h1>
                </button>
            </div>
        </>
    );
